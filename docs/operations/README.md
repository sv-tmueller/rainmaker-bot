...
`--since` restricted to the active calibration regime's start date (see the
`REGIME_SINCE` env var set in the diagnostics step of `daily-diagnostics.yml`).
...
`backfill` re-run), update the `REGIME_SINCE` value in `daily-diagnostics.yml`
in the same change that records the refit.
