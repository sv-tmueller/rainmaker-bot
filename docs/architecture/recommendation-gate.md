...
## Update 2026-09-04: Hot-city TMAX calibration audit (#373, batch #370)

### Context

The venue-decomposition diagnostic (#372) found that Los Angeles, San
Francisco, Dallas, and Houston lose heavily on Polymarket NO-side TMAX bets:
the engine bets the high stays below a bucket, then the actual realizes above
it, suggesting the forecast mean is systematically low for summer highs in
these cities. Houston was already excluded (see `STATION_POLICIES["KHOU"]`
above); this audit covers all four.

### Methodology

Analysis script: `scripts/tmax_bias_audit.py`. Computes bias per (city, lead)
as `mean(forecast_mu - actual)` from the `predictions` table's `dist_params`
JSON (`mu` field) joined to `outcomes.actual_value`, deduplicated to the latest
run per (market, UTC day) — the same deduplication `compute_live_accuracy` in
`tracking.py` uses. Cross-checked against the `forecast_accuracy` table's
`bias_f` column (mean signed error, mu − actual).

Thresholds:
- **Gate**: |bias| ≥ 3.0°F and structural (≥70% same-sign, std-error <
  1.5×|mean-bias|)
- **Tighten**: |bias| ≥ 1.5°F and structural
- **Refit**: |bias| ≥ 1.5°F and episodic (mixed signs, high variance)
- **Monitor**: |bias| < 1.5°F

### Data limitations

The local SQLite DB (`rainmaker.db`) has **limited settled history**: 8 days
(August 23–30, 2026), lead 0 only, generated by `backfill` runs (single-source
Open-Meteo, `n_sources=1`). There are **no calibration cells** fitted (the
`calibration` table is empty), so all `dist_params.mu` values are uncalibrated
raw forecast means. The production Supabase store (via `DATABASE_URL`) has
months of multi-lead settled history with calibrated predictions; the full audit
requires a production run. These local results demonstrate the methodology and
provide preliminary directional findings.

### Results: bias per (city, lead 0)

Compared against `forecast_accuracy` table (kind=`live`):

| City | ICAO | N | Mean μ (°F) | Mean Actual (°F) | Bias (μ−actual) | MAE | Accur. bias_f | % Too Warm | Consistency |
| --- | --- | -: | -: | -: | -: | -: | -: | -: | --- |
| Los Angeles | KLAX | 8 | 88.79 | 84.75 | **+4.04** | 4.04 | +4.04 | 100% | structural |
| San Francisco | KSFO | 8 | 73.18 | 71.25 | **+1.93** | 2.52 | +1.93 | 75% | structural |
| Dallas | KDAL | 7 | 101.04 | 101.57 | **−0.54** | 0.79 | −0.54 | 43% | episodic |
| Houston | KHOU | 8 | 97.20 | 96.75 | **+0.45** | 1.31 | +0.45 | 63% | mostly structural |

Positive bias = forecast mean is **too warm** (engine predicts hotter than
actual). Negative bias = forecast mean is **too cold** (engine predicts cooler
than actual).

### Key finding: direction reversal vs venue-decomp hypothesis

The issue hypothesized a **cold bias** (forecast mean too low → engine bets
the high stays below a bucket → actual goes above). However, the local
settled-history data shows the **opposite** for LA and SF:

- **Los Angeles**: +4.04°F **warm** bias, 100% of days too warm, structurally
  consistent. The forecast mean is systematically HIGH, not low. If the engine
  bets NO on high buckets (expects temps stay below), a warm-biased forecast
  would actually push toward YES on high buckets — so the NO-side losses may
  stem from a different mechanism: insufficient spread (sigma too tight,
  underestimating tail probability) rather than a cold mean bias. The sigma
  values for LA range 3.8–7.3°F, which seem reasonable, but the bucket
  boundaries may sit far from the mean, making tail probabilities sensitive to
  small sigma misestimation.

- **San Francisco**: +1.93°F warm bias, 75% of days too warm, structural. Same
  direction as LA but smaller magnitude. Already has a +0.05 edge-floor delta
  (`STATION_EDGE_DELTA["KSFO"]["TMAX"]`).

- **Dallas**: −0.54°F slight cold bias, episodic (alternating signs). Within
  noise; no significant bias detected.

- **Houston**: +0.45°F slight warm bias, mostly structural but small magnitude.
  Already excluded via `STATION_POLICIES`.

The discrepancy between the venue-decomp finding (loses on NO-side TMAX =
suggests cold bias) and the raw forecast-vs-actual data (shows warm bias for
LA/SF) implies the NO-side losses are driven primarily by **spread/sigma
misestimation**, not mean bias. The engine's Gaussian may be too narrow in the
tails, causing it to underestimate the probability of temperatures exceeding
upper bucket boundaries even when the mean is correctly placed (or even
warm-biased). This is consistent with the known overconfidence finding from the
original backtest (#59): central-interval coverage was 39/67/78% against nominal
50/80/90%, meaning the raw Gaussian is too narrow.

### Per-city recommendations

#### Los Angeles (KLAX): **REFIT**

- **Bias**: +4.04°F warm, structural, 100% same-sign
- **Note on script-vs-record divergence**: The analysis script's `recommend()`
  function treats warm and cold bias symmetrically at the gate threshold
  (|bias| >= 3.0°F and structural -> "gate"). Given LA's +4.04°F warm bias,
  the script outputs "gate". This record overrides that to "refit" because a
  warm bias is correctable via calibration (the calibration cell is empty, so
  fitting the bias parameter addresses the root cause directly), whereas
  gating would discard a city whose only defect is an unfitted bias parameter.
  The script's symmetric gate logic is designed for cold biases where the
  forecast-dominant bust pattern (see KNYC exclusion) makes calibration unable
  to fix the problem; a warm bias from an empty calibration cell does not
  share that pathology.
- **Recommendation**: Refit the calibration cell for `(KLAX, TMAX, *)`.
- **Root cause**: The forecast mean is systematically 4°F too warm. Since the
  calibration table is empty (no cells fitted), the engine is running
  uncalibrated — applying `UNCALIBRATED_WIDEN` (1.25×) to the raw spread but no
  bias correction. Running `uv run rainmaker backfill --city "Los Angeles"`
  would fit the bias parameter and shift the predictive mean down by ~4°F.
- **Specific calibration parameter and cell**:
  - Cell key: `(station='KLAX', variable='TMAX', lead_time=N)` for each lead
    N ∈ {0,1,2,3}.
  - Parameter: `bias` column in the `calibration` table.
  - Mechanism: `apply_calibration` computes `predictive_mean = mu − bias`. With
    a fitted bias of ≈+4.0, the predictive mean drops from 88.79 to 84.79,
    closely matching the actual mean of 84.75.
  - Fit command: `uv run rainmaker backfill --city "Los Angeles"`.
  - Caveat: the local backfill data is lead-0 only and single-source; the
    production backfill pulls 45 days of multi-lead history from the Open-Meteo
    Previous Runs API, producing a more robust fit. The production fit is the
    one that matters.

#### San Francisco (KSFO): **TIGHTEN**

- **Bias**: +1.93°F warm, structural, 75% same-sign
- **Recommendation**: Tighten — increase the existing edge-floor delta.
- **Current policy**: `STATION_EDGE_DELTA[("KSFO", "TMAX")] = 0.05` (from #303,
  the 2026-07-18 update).
- **Action**: Increase the edge-floor delta from 0.05 to 0.10 to further reduce
  exposure until a calibration refit can be evaluated. The bias is structural
  but moderate; a refit is also viable but the tighten path is lower-risk since
  the delta is already in place and reversible.
- **Alternative**: If the team prefers refit over tighten, the calibration cell
  is `(station='KSFO', variable='TMAX', lead_time=N)`; fitting a bias of ≈+1.93
  would correct the warm drift. Command: `uv run rainmaker backfill --city "San
  Francisco"`.

#### Dallas (KDAL): **MONITOR**

- **Bias**: −0.54°F slight cold, episodic, alternating signs
- **Recommendation**: Monitor — no action needed. The bias is within noise
  (|bias| < 1.0°F, std-error 1.09°F). The episodic pattern (some days warm, some
  cold) suggests no structural defect. Continue collecting settled history and
  re-evaluate after the production run yields more data across multiple leads.

#### Houston (KHOU): **MONITOR** (already gated)

- **Bias**: +0.45°F slight warm, mostly structural but small magnitude
- **Recommendation**: Monitor — already excluded via `STATION_POLICIES["KHOU"]`.
  The local data shows negligible bias (+0.45°F), which contradicts the
  venue-decomp finding (−25% ROI, 37% hit rate). Two possible explanations:
  1. The local 8-day window (late August) doesn't capture the seasonal/regime
     pattern causing the venue-decomp losses.
  2. The venue-decomp losses stem from spread/sigma misestimation rather than
     mean bias, as noted above.
  **Path back in**: Re-run this audit against the production store
  (`DATABASE_URL` set) to get multi-month, multi-lead data before deciding
  whether to lift the exclusion.

### Spread/sigma observations

The sigmas in the local data vary widely:

| City | σ range (°F) | Mean σ | Notes |
| --- | --- | -: | --- |
| Los Angeles | 3.84 – 7.30 | 5.51 | Wide but still warm-biased; sigma not compensating |
| San Francisco | 1.98 – 4.25 | 3.05 | Narrow; tail sensitivity high |
| Dallas | 1.88 – 3.40 | 2.78 | Reasonable; smallest bias |
| Houston | 1.50 – 4.08 | 2.53 | Several rows at σ=1.5 (the MIN_SIGMA_F floor) |

SF and Houston hitting the σ=1.5 floor (`MIN_SIGMA_F`) on multiple days is a
red flag: the raw ensemble spread is being floored, meaning the models agree
too closely and the predictive distribution is artificially narrow. This
directly contributes to tail-probability misestimation on NO-side bets.

### Production run requirements

To complete the full audit, run `scripts/tmax_bias_audit.py` with
`DATABASE_URL` set to the Supabase Postgres DSN:

```bash
DATABASE_URL="postgresql://..." uv run python scripts/tmax_bias_audit.py
```

The production store has:
- Months of settled history (vs 8 days locally)
- Multiple leads (0–3, vs lead-0 only locally)
- Calibrated predictions (calibration cells fitted, vs empty locally)
- Multi-source forecasts (NWS + Open-Meteo, vs single-source locally)

The production run will produce definitive bias magnitudes per (city, lead) and
may reverse the preliminary recommendations above.
